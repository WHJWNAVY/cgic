...
typedef enum { cgiParseSuccess, cgiParseMemory, cgiParseIO } cgiParseResultType;
...
int main(int argc, char *argv[]) {
    ...
    if (cgiHeaderChk() != 0) {
        CGIC_DEBUG("Header check failed!\n");
        return -1;
    }
#endif
...
static void decomposeValue(char *value, char *mvalue, int mvalueSpace, char **argNames, char **argValues,
                           int argValueSpace);
...
cgiFormResultType cgiFormFileFullName(char *name, char *result, int resultSpace) {
    cgiFormEntry *e;
    int resultLen = 0;
    char *s;
    e = cgiFormEntryFindFirst(name);
...
void cgiHeaderCookieSet(char *name, char *value, int secondsToLive, char *path, char *domain, int options) {
	...
	*/
    ...
    gt = gmtime(&then);
    fprintf(cgiOut, "Set-Cookie: %s=%s; domain=%s; expires=%s, %02d-%s-%04d %02d:%02d:%02d GMT; path=%s%s%s%s\r\n",
            name, value, domain, days[gt->tm_wday], gt->tm_mday, months[gt->tm_mon], gt->tm_year + 1900, gt->tm_hour,
            gt->tm_min, gt->tm_sec, path, ((options & cgiCookieSecure) ? "; Secure" : ""),
...
cgiEnvironmentResultType cgiWriteEnvironment(char *filename) {
    ...
    while (e) {
        ...
        if (cgiFormFileOpen(e->attr, &fp) == cgiFormSuccess) {
            ...
            while (cgiFormFileRead(fp, buffer, sizeof(buffer), &got) == cgiFormSuccess) {
...
cgiEnvironmentResultType cgiReadEnvironment(char *filename) {
    ...
    while (1) {
        ...
        if (fileFlag) {
            ...
            if (getTempFile(&out, NULL) != cgiParseSuccess || !out) {
