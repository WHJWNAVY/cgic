...
char *flavors[] = {"pistachio", "walnut", "creme"};
...
void Flavors() {
    ...
    int result;
    int invalid;
    result = cgiFormSelectMultiple("flavors", flavors, 3, flavorChoices, &invalid);
...
void Cookies() {
    ...
    if (strlen(cname)) {
        ...
        fprintf(cgiOut, "<p>\n");
        fprintf(cgiOut, "If your browser accepts cookies (many do not), this new cookie should appear in the above "
                        "list the next time the form is submitted.<p>\n");
    }
    cgiStringArrayFree(array);
}
...
void File() {
    ...
    fprintf(cgiOut, "<p>\n");
    fprintf(cgiOut, "Of course, this is only the claim the browser made when uploading the file. Much like the "
                    "filename, it cannot be trusted.<p>\n");
...
void ShowForm() {
    ...
    fprintf(cgiOut, "Pressing this button will submit the form, then save the CGI environment so that it can be "
                    "replayed later by calling cgiReadEnvironment (in a debugger, for instance).<p>\n");
...
void LoadEnvironment() {
    if (cgiReadEnvironment(SAVED_ENVIRONMENT) != cgiEnvironmentSuccess) {
        cgiHeaderContentType("text/html");
...
void SaveEnvironment() {
    if (cgiWriteEnvironment(SAVED_ENVIRONMENT) != cgiEnvironmentSuccess) {
        fprintf(cgiOut,
                "<p>cgiWriteEnvironment failed. Most "
